...
def divide_label_unlabel_voc(dataset_dicts, labeled_path,medium_labeled_path):
    ...
    clabeled_dict = split_dict['clabeled']
    if 'meidum' in split_dict:
        medium_dict = split_dict['meidum']
    else:
        medium_dict = split_dict['medium']
    ...
    for i in range(len(dataset_dicts)):
        data_item = dataset_dicts[i]
        if data_item['image_id'] in clabeled_dict or data_item['image_id'] in medium_dict:
            label_dicts.append(dataset_dicts[i])
        else:
            unlabel_dicts.append(dataset_dicts[i])

        if data_item['image_id'] in medium_dict:
            unlabel_dicts.append(dataset_dicts[i])
    print(len(label_dicts))
    print(len(unlabel_dicts))

    return label_dicts, unlabel_dicts

def divide_cls_reg_label_unlabel_voc(dataset_dicts_cls, dataset_dicts_reg,cls_labeled_path,reg_labeled_path):


    with open(cls_labeled_path,'rb') as f:
        cls_labeled_id_list= pickle.load(f)

    with open(reg_labeled_path,'rb') as f:
        reg_labeled_id_list= pickle.load(f)


    # read from pre-generated data seed
    
    clabeled_dict_cls = cls_labeled_id_list['clabeled']
    if 'meidum' in cls_labeled_id_list:
        medium_dict_cls = cls_labeled_id_list['meidum']
    else:
        medium_dict_cls = cls_labeled_id_list['medium']


    clabeled_dict_reg = reg_labeled_id_list['clabeled']
    if 'meidum' in reg_labeled_id_list:
        medium_dict_reg = reg_labeled_id_list['meidum']
    else:
        medium_dict_reg = reg_labeled_id_list['medium']
    ...
    for i in range(len(dataset_dicts_cls)):
        data_item = dataset_dicts_cls[i]
        if data_item['image_id'] in clabeled_dict_cls or data_item['image_id'] in medium_dict_cls:
            cls_label_dicts.append(dataset_dicts_cls[i])


        # if data_item['image_id'] in medium_dict_cls:
        unlabel_dicts.append(dataset_dicts_cls[i])

    for i in range(len(dataset_dicts_reg)):
        data_item = dataset_dicts_reg[i]
        if data_item['image_id'] in clabeled_dict_reg or data_item['image_id'] in medium_dict_reg:
            reg_label_dicts.append(dataset_dicts_reg[i])

    
    print(len(cls_label_dicts))
    print(len(reg_label_dicts))
    print(len(unlabel_dicts))
    return cls_label_dicts, reg_label_dicts,unlabel_dicts
...
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)
        ...
        # Divide into labeled and unlabeled sets according to supervision percentage
        label_dicts, unlabel_dicts = divide_label_unlabel_voc(
            dataset_dicts,
            cfg.DATALOADER.RANDOM_DATA_SEED_PATH,
            cfg.DATALOADER.RANDOM_DATA_SEED_PATH_MEDIUM
        )
...
def build_detection_semisup_train_loader_three_crops(cfg, mapper=None):
    if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)
        ...
        dataset_dicts_cls = get_detection_dataset_dicts(
            cfg.DATASETS.TRAIN_CLS,
            filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
            min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
            if cfg.MODEL.KEYPOINT_ON
            else 0,
        ...
        dataset_dicts_reg = get_detection_dataset_dicts(
            cfg.DATASETS.TRAIN_REG,
            filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
            min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
            if cfg.MODEL.KEYPOINT_ON
            else 0,
        ...
        # Divide into labeled and unlabeled sets according to supervision percentage
        cls_label_dicts, reg_label_dicts,unlabel_dicts = divide_cls_reg_label_unlabel_voc(
            dataset_dicts_cls,
            dataset_dicts_reg,
            cfg.DATALOADER.RANDOM_DATA_SEED_PATH_CLS,
            cfg.DATALOADER.RANDOM_DATA_SEED_PATH_REG
        )

    cls_label_dataset = DatasetFromList(cls_label_dicts, copy=False)

    reg_label_dataset = DatasetFromList(reg_label_dicts, copy=False)
    # exclude the labeled set from unlabeled dataset
    unlabel_dataset = DatasetFromList(unlabel_dicts, copy=False)
    # include the labeled set in unlabel dataset
    # unlabel_dataset = DatasetFromList(dataset_dicts, copy=False)
    ...
    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    cls_label_dataset = MapDataset(cls_label_dataset, mapper)
    reg_label_dataset = MapDataset(reg_label_dataset, mapper)
    unlabel_dataset = MapDataset(unlabel_dataset, mapper)
    ...
    logger.info("Using training sampler {}".format(sampler_name))
    if sampler_name == "TrainingSampler":
        cls_label_sampler = TrainingSampler(len(cls_label_dataset))
        reg_label_sampler = TrainingSampler(len(reg_label_dataset))
        unlabel_sampler = TrainingSampler(len(unlabel_dataset))
    elif sampler_name == "RepeatFactorTrainingSampler":
        raise NotImplementedError("{} not yet supported.".format(sampler_name))
    else:
    ...
    return build_semisup_batch_data_loader_three_crop(
        (cls_label_dataset, reg_label_dataset, unlabel_dataset),
        (cls_label_sampler, reg_label_sampler,unlabel_sampler),
        cfg.SOLVER.IMG_PER_BATCH_LABEL,
        cfg.SOLVER.IMG_PER_BATCH_UNLABEL,
        aspect_ratio_grouping=cfg.DATALOADER.ASPECT_RATIO_GROUPING,
        num_workers=cfg.DATALOADER.NUM_WORKERS,
...
def build_semisup_batch_data_loader_three_crop(
    ...
    *,
    ...
    cls_label_dataset, reg_label_dataset, unlabel_dataset = dataset
    cls_label_sampler, reg_label_sampler, unlabel_sampler = sampler

    if aspect_ratio_grouping:
        cls_label_data_loader = torch.utils.data.DataLoader(
            cls_label_dataset,
            sampler=cls_label_sampler,
            num_workers=num_workers,
            batch_sampler=None,
            collate_fn=operator.itemgetter(
                0
        ...
        reg_label_data_loader = torch.utils.data.DataLoader(
            reg_label_dataset,
            sampler=reg_label_sampler,
            num_workers=num_workers,
            batch_sampler=None,
            collate_fn=operator.itemgetter(
                0
        ...
        return AspectRatioGroupedSemiSupDatasetThreeCrop(
            (cls_label_data_loader, reg_label_data_loader,unlabel_data_loader),
            (batch_size_label, batch_size_unlabel),
        )
    else:
        raise NotImplementedError("ASPECT_RATIO_GROUPING = False is not supported yet")