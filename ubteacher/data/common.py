...
class AspectRatioGroupedSemiSupDatasetThreeCrop(AspectRatioGroupedDataset):
    ...
    def __init__(self, dataset, batch_size):
        ...
        self.cls_label_dataset, self.reg_label_dataset, self.unlabel_dataset = dataset
        self.batch_size_label = batch_size[0]
        self.batch_size_unlabel = batch_size[1]

        self.cls_label_buckets = [[] for _ in range(2)]
        self.cls_label_buckets_key = [[] for _ in range(2)]
        self.reg_label_buckets = [[] for _ in range(2)]
        self.reg_label_buckets_key = [[] for _ in range(2)]
        self._unlabel_buckets = [[] for _ in range(2)]
        self._unlabel_buckets_key = [[] for _ in range(2)]
        # Hard-coded two aspect ratio groups: w > h and w < h.
        # Can add support for more aspect ratio groups, but doesn't seem useful
    ...
    def __iter__(self):
        cls_label_bucket, reg_label_bucket, unlabel_bucket = [], [], []

        for d_cls_label, d_reg_label, d_unlabel in zip(self.cls_label_dataset, self.reg_label_dataset, self.unlabel_dataset):
            # d is a tuple with len = 2
            # It's two images (same size) from the same image instance
            # d[0] is with strong augmentation, d[1] is with weak augmentation
            ...
            # label and unlabel buckets might not have the same number of data
            # i.e., one could reach batch_size, while the other is still not
            if len(cls_label_bucket) != self.batch_size_label:
                w, h = d_cls_label[0]["width"], d_cls_label[0]["height"]
                cls_label_bucket_id = 0 if w > h else 1
                cls_label_bucket = self.cls_label_buckets[cls_label_bucket_id]
                cls_label_bucket.append(d_cls_label[0])
                cls_label_buckets_key = self.cls_label_buckets_key[cls_label_bucket_id]
                cls_label_buckets_key.append(d_cls_label[1])

            if len(reg_label_bucket) != self.batch_size_label:
                w, h = d_reg_label[0]["width"], d_reg_label[0]["height"]
                reg_label_bucket_id = 0 if w > h else 1
                reg_label_bucket = self.reg_label_buckets[reg_label_bucket_id]
                reg_label_bucket.append(d_reg_label[0])
                reg_label_buckets_key = self.reg_label_buckets_key[reg_label_bucket_id]
                reg_label_buckets_key.append(d_reg_label[1])
            ...
            # yield the batch of data until all buckets are full
            if (
                len(cls_label_bucket) == self.batch_size_label
                and len(reg_label_bucket) == self.batch_size_label
                and len(unlabel_bucket) == self.batch_size_unlabel
            ):
                # label_strong, label_weak, unlabed_strong, unlabled_weak
                yield (
                    cls_label_bucket[:],
                    cls_label_buckets_key[:],
                    reg_label_bucket[:],
                    reg_label_buckets_key[:],
                    unlabel_bucket[:],
                    unlabel_buckets_key[:],
                )
                del cls_label_bucket[:]
                del cls_label_buckets_key[:]
                del reg_label_bucket[:]
                del reg_label_buckets_key[:]
                del unlabel_bucket[:]
                del unlabel_buckets_key[:]
